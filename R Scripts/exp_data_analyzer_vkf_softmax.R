# Script to use the VKF + softmax (referred to as stepping stone model in the report) to analyze the experimental data
...
DElower <- setNames(c(0.01, 0.01, 0.01, 0.01, 0.01), 
                    c("m0", "v0", "lambda", "omega", "beta"))
DEupper <- setNames(c(3, 3, 0.99, 3, 3),
                    c("m0", "v0", "lambda", "omega", "beta"))
...
wrapper <- function(pars, data, nTrials, nChoices) {
  pars <- setNames(pars, c("m0", "v0", "lambda", "omega", "beta"))
...
for(participant in 4:nParticipants) {
  ...
  save_dir <- paste0("../Data/Fitted Models/Stepping Stone Model")
  ...
  pars <- setNames(out$optim$bestmem, c("m0", "v0", "lambda", "omega", "beta"))
...
generateData <- function(filename_participant_model) {
  ...
  for(i in 2:(nTrials+1)) {
    ...
    pChoice <- exp(beta*temp_M)/sum(exp(beta*temp_M))
...
for(participant in 1:nParticipants) {
  ...
  save_dir <- paste0("../Data/Simulated Data/Stepping Stone Model Experiment Simulation")
  if(!dir.exists(save_dir)) dir.create(save_dir, recursive=TRUE)
  for(i in 1:nSim) {
    print(paste0("Generating data for participant ", participant, ", sim ", i)) # Have some console output to see it's working
    sim_data[[i]] <- generateData(paste0("../Data/Fitted Models/Stepping Stone Model/Participant-", participant, ".RData"))
...
for(participant in 1:nParticipants) {
  ...
  load(paste0("../Data/Fitted Models/Stepping Stone Model/Participant-", participant, ".RData"))
  load(paste0("../Data/Simulated Data/Stepping Stone Model Experiment Simulation/Participant-", participant, ".RData"))
...
for(participant in 1:nParticipants) {
  ...
  load(paste0("../Data/Fitted Models/Stepping Stone Model/Participant-", participant, ".RData")) # For checking reversal point sentinel
  accuracy <- matrix(NA, nrow=nBlocks, ncol=nBins)
  for (block in 1:nBlocks) {
...
fig9 <- arrangeGrob(plots[[1]], plots[[2]], plots[[3]], plots[[4]],
                    ncol=4, nrow=1, top="Stepping Stone Model Fit to Data")
ggsave("../Figures/fig9.png", plot=fig9, width = 4 * 400, height = 1 * 400 + 20, units = "px", dpi=72)